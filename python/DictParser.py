...
class DictParser(object):
    ...
    def parseName(self):
        if self.state == State.END:
            return None
        if self.state in [State.START, State.CURLY]:
            self.matchOpenCurly()
        if self.state == State.NAME_START:
            self.matchNameStart()
    ...
    def parseValue(self):
        ...
        if self.state == State.SIMPLE:
            val = self.parseSimpleValue()
        elif self.state == State.BINARY_SIZE:
            val = self.parseBinaryValue()
        self.matchNameStart()
        return val
    ...
    def parseNameValue(self):
        if self.state == State.END:
            return None
        if self.state != State.NAME_DATA:
            self.barf("invalid state: not at name data")
    ...
    def parseBinaryData(self, size):
        ...
        if len(val) != size:
            self.barf("unexpected EOF when reading binary property value")
        return val

    def matchOpenCurly(self):
        c = self.peekOrBarf("unexpected EOF when reading open curly")
        if c != b'{':
            self.barf("missing initial '{'")
        self.input.read(1)
        self.state = State.NAME_START

    def matchNameStart(self):
        if self.state != State.NAME_START:
            self.barf("invalid state: not at name start")
